        ...
        <nav className="hidden md:flex items-center gap-8" aria-label="Main navigation">
          {NAV_LINKS.map(link => (
            <Link
              key={link.href}
              href={link.href}
              className="relative text-sm text-white/70 hover:text-white transition-colors tracking-wide"
            >
              {link.label}
              {isActive(link.href) && (
                <motion.span
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-ast-dark/98 flex flex-col items-center justify-center gap-8"
          >
            {NAV_LINKS.map((link, i) => (
              ...
              >
                <Link
                  ...
                  className="font-heading text-3xl font-bold text-white hover:text-ast-light transition-colors"
                >
                  {link.label}
                </Link>
              </motion.div>
            ...
            <motion.div
              ...
              className="flex flex-col gap-3 mt-4 w-56"
            >
              {user ? (
                <>
